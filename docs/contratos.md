...
**Jimi:** `payload.Imei` = IMEI Concox (fallback `deviceId` si legacy null) → lookup `Dispositivos.Imei` (`resolveByImei`). `Combustible` litros float → `Posiciones.Combustible` (float). Alarm AMQP solo SOS (`jimi.alarm.*`); powercut/battery van en `jimi.position`.

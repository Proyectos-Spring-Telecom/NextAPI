...
@Injectable()
export class CatEstatusDispositivoService {
  constructor(
    @InjectRepository(CatEstatusDispositivo)
    private readonly repository: Repository<CatEstatusDispositivo>,
    private readonly bitacoraLogger: BitacoraLoggerService,
  ) {}

  async create(
    dto: CreateCatEstatusDispositivoDto,
    idUser: number,
    ...
    try {
      ...
      if (existe) {
        throw new BadRequestException('El estatus de dispositivo ya existe');
      }
      ...
      await this.bitacoraLogger.logToBitacora(
        ...
        `Se creó el estatus de dispositivo: ${dto.nombre}`,
        'CREATE',
        { dto },
        idUser,
      ...
      return {
        status: 'success',
        message: 'Estatus de dispositivo creado correctamente',
        data: { id: Number(saved.id), nombre: saved.nombre },
      };
    } catch (error) {
      await this.bitacoraLogger.logToBitacora(
        'CatEstatusDispositivo',
        `Error al crear estatus de dispositivo: ${dto.nombre}`,
        'CREATE',
        { dto },
        idUser,
        ID_MODULO_DISPOSITIVOS,
  ...
  async findOne(id: number): Promise<{ data: CatEstatusDispositivo }> {
    try {
      const entity = await this.repository.findOne({ where: { id } });
      if (!entity) {
        throw new NotFoundException('Estatus de dispositivo no encontrado');
      }
      return {
      ...
      throw new HttpException(
        { message: 'Error al buscar el estatus de dispositivo' },
  ...
  async update(
    id: number,
    dto: UpdateCatEstatusDispositivoDto,
    idUser: number,
  ): Promise<ApiCrudResponse> {
    try {
      const entity = await this.repository.findOne({ where: { id } });
      ...
      await this.bitacoraLogger.logToBitacora(
        'CatEstatusDispositivo',
        `Se actualizó el estatus de dispositivo ID: ${id}`,
        'UPDATE',
        { id, dto },
        idUser,
        ID_MODULO_DISPOSITIVOS,
      ...
      return {
        status: 'success',
        message: 'Estatus de dispositivo actualizado correctamente',
  ...
  async updateEstatus(
    id: number,
    dto: UpdateCatEstatusDispositivoEstatusDto,
    idUser: number,
  ): Promise<ApiCrudResponse> {
    try {
      const entity = await this.repository.findOne({ where: { id } });
      ...
      throw new InternalServerErrorException(
        `Error al cambiar estatus del estatus de dispositivo`,
      );
    }
  }
}

...
export class CatEstatusSimController {
  constructor(
    private readonly catEstatusSimService: CatEstatusSimService,
  ) {}

  @Post()
  @ApiOperation({ summary: 'Crear estatus de SIM' })
  @ApiResponse({ status: 201, description: 'Estatus creado correctamente' })
  @ApiResponse({ status: 400, description: 'Datos inválidos' })
  @ApiResponse({ status: 401, description: 'No autorizado' })
  async create(
    @Body() dto: CreateCatEstatusSimDto,
    @Request() req,
  ): Promise<ApiCrudResponse> {
    const idUser = req.user.userId;
    return this.catEstatusSimService.create(dto, idUser);
  }

  @Get('list')
  @ApiOperation({ summary: 'Lista completa de estatus de SIM' })
  @ApiQuery({
    name: 'soloActivos',
    required: false,
  ...
  async findAllList(
    ...
    return this.catEstatusSimService.findAllList(soloActivosBool);
  }

  @Get(':page/:limit')
  @ApiOperation({ summary: 'Lista paginada de estatus de SIM' })
  ...
  async findAll(
    ...
    return this.catEstatusSimService.findAll(
      page,
      limit,
      soloActivosBool,
    );
  ...
  async findOne(@Param('id', ParseIntPipe) id: number) {
    return this.catEstatusSimService.findOne(id);
  }

  @Put(':id')
  @ApiOperation({ summary: 'Actualizar estatus de SIM' })
  @ApiParam({ name: 'id', description: 'ID del estatus' })
  ...
  async update(
    ...
    @Body() dto: UpdateCatEstatusSimDto,
    @Request() req,
  ): Promise<ApiCrudResponse> {
    const idUser = req.user.userId;
    return this.catEstatusSimService.update(id, dto, idUser);
  }
  ...
  async updateEstatus(
    @Param('id', ParseIntPipe) id: number,
    @Body() dto: UpdateCatEstatusSimEstatusDto,
    @Request() req,
  ): Promise<ApiCrudResponse> {
    const idUser = req.user.userId;
    return this.catEstatusSimService.updateEstatus(id, dto, idUser);
  }
}
